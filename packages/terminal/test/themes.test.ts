...
describe('themes', () => {
  ...
  it.each(themeNames)(
    'theme "%s" exists in themes, terminalThemes, and cssThemeProperties',
    (name) => {
  ...
  it.each(themeNames)(
    'theme "%s" has all CssThemeProperties fields as non-empty strings',
    (name) => {
  ...
  it.each(themeNames)(
    'theme "%s" has all TerminalThemeColors fields as valid hex strings',
    (name) => {
      ...
      for (const field of TERMINAL_FIELDS) {
        ...
        expect(
          HEX_RE.test(value),
          `${name}.terminal.${field} = "${value}" is not a valid hex color`,
        ).toBe(true);
      }
    },
  );
