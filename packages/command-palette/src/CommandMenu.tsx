...
import type { CommandContext, CommandItem, CommandMenuProps } from './types';
      ...
      >
        ...
        >
          ...
          <div className="flex shrink-0 items-center gap-2.5 border-t border-border px-3 py-2 text-[11px] text-text-muted opacity-60">
            ...
            <span>·</span>
            <span>⌫ {drillStack.length > 0 || section !== 'root' ? 'back' : '—'}</span>
            <span>·</span>
            <span>Esc {drillStack.length > 0 ? 'back' : 'close'}</span>
          </div>
        </Dialog>
      </Modal>
    </ModalOverlay>
