...
const checkbox = tv({
  ...
  variants: {
    checked: { true: 'border-danger/50 bg-danger/10', false: 'border-edge/50 bg-base/50' },
  },
});
    ...
    >
      <label className="mt-4 flex cursor-pointer items-start gap-2.5">
        <div className={checkbox({ checked: deleteGit })}>
          {deleteGit && <Check size={10} strokeWidth={2.5} className="text-danger" />}
        </div>
        <input
          type="checkbox"
          checked={deleteGit}
      ...
      {deleteGit && (
        <p className="mt-2 text-sm leading-relaxed text-danger/80">
          Runs <code className="rounded-sm bg-surface px-1 font-mono">git worktree remove</code> and{' '}
          <code className="rounded-sm bg-surface px-1 font-mono">git branch -d</code>. Uncommitted
