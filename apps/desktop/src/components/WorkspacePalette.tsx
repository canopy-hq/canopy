...
const tabButton = tv({
  base: 'flex flex-1 items-center justify-center gap-1 rounded border-none px-2 py-1.25 text-md',
  variants: {
      ...
      <Modal className="w-[440px] overflow-hidden rounded-[10px] border border-border bg-bg-primary shadow-[0_20px_60px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex items-center gap-2 border-b border-border px-4 py-3">
          ...
          <input
            ...
            className="flex-1 border-none bg-transparent text-lg text-text-primary outline-none"
          />
          <Kbd>ESC</Kbd>
        </div>
        ...
        {isCreateMode && !pickingBase && (
          <div className="mx-2 mt-2 rounded-lg border border-accent/[0.15] bg-accent/[0.05] p-3">
            <div className="mb-2 flex items-center gap-2">
              ...
              <span className="flex-1 text-base font-medium text-accent">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-[5px] border border-border bg-bg-tertiary px-2 py-0.5 text-sm"
              >
                <svg
                  width="9"
                  height="9"
        ...
        <div className="mx-2 mt-2 flex gap-0.5 rounded-[6px] bg-bg-primary p-0.5">
          <button onClick={() => setTab('all')} className={tabButton({ active: tab === 'all' })}>
            All{' '}
            <span className="rounded-full bg-white/[0.06] px-1.25 text-xs">{branches.length}</span>
          </button>
          <button
          ...
          >
            Worktrees{' '}
            <span className="rounded-full bg-white/[0.06] px-1.25 text-xs">
        ...
        <div className="max-h-[320px] overflow-y-auto px-2 pb-2">
          {tab === 'all' && !pickingBase && (
            <>
              <div className="px-2 pt-1.5 pb-1 text-xs tracking-[0.5px] text-text-muted uppercase">
                Branches
              </div>
              {filteredBranches.length === 0 && (
                <div className="py-3 text-center text-md text-text-muted">
              ...
              <div className="mt-1 border-t border-border px-2 pt-1.5 pb-1 text-xs tracking-[0.5px] text-text-muted uppercase">
                Worktrees
              </div>
              {filteredWorktrees.length === 0 && (
                <div className="py-3 text-center text-md text-text-muted">No worktrees</div>
              )}
            ...
            <>
              {filteredWorktrees.length === 0 && (
                <div className="px-4 py-8 text-center">
                  <div className="mb-1 text-base text-text-muted">No worktrees</div>
                  <div className="text-md text-text-muted">Create one from the All tab</div>
                </div>
              )}
              {filteredWorktrees.map((wt) => (
                <WorktreeItem
              ...
              {filteredWorktrees.length > 0 && (
                <div className="mt-1 border-t border-border p-2 text-center text-md text-text-muted">
              ...
              {branches
                ...
                .map((b) => (
                  ...
                  >
                    ...
                    <span
                      className={`text-base text-text-primary ${b.name === baseBranch ? 'font-medium' : 'font-normal'}`}
                    >
                      {b.name}
                    </span>
        ...
        {/* Footer */}
        <div className="flex items-center gap-3 border-t border-border px-4 py-2 text-sm text-text-muted">
          {isCreateMode ? (
            <>
              <span className="flex items-center gap-0.5">
                <Kbd>⌘</Kbd>
  ...
  return (
    ...
    >
      <div className={branchItemRow({ disabled, isConfirming })}>
        ...
        <span
          className={`text-base text-text-primary ${branch.is_head ? 'font-medium' : 'font-normal'}`}
        >
          {branch.name}
        </span>
      ...
      {isConfirming && (
        <div className="border-t border-accent/10 bg-accent/[0.03] px-2.5 py-2">
          <div className="mb-1.5 text-sm text-text-muted">
            Create worktree for <strong className="text-text-primary">{branch.name}</strong>
          </div>
          <div className="mb-2 flex items-center gap-1.5 rounded bg-bg-primary px-2 py-1 font-mono text-sm text-text-muted">
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="text-base text-text-primary">{worktree.name}</div>
        {showPath && (
          <div className="overflow-hidden text-xs text-ellipsis whitespace-nowrap text-text-muted">
            {worktree.path}
          </div>
        )}
      </div>
      {isInSidebar ? (
        <span className="ml-auto text-sm text-text-muted">opened</span>
      ) : (
        <Button
          size="sm"
